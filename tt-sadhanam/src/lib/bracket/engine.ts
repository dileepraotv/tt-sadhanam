...
export function generateBracket(
  ...
  // Seeded players are sorted by their seed value (ascending = best first).
  // They are assigned RELATIVE ranks 1, 2, 3... regardless of absolute seed
  // numbers, so seeds like [3, 10, 50] map correctly to ranks [1, 2, 3].
  // This means any positive seed value works as a ranking indicator.
  const seeded = players
    .filter(p => p.seed != null && p.seed >= 1)
    .sort((a, b) => (a.seed as number) - (b.seed as number))

  const unseeded = shuffle(
    players.filter(p => !p.seed || p.seed < 1),
    rng,
  )

  // Build rank → player map (1-indexed, relative ranks).
  // Rank 1 = best seed, ranks K+1..N = unseeded players in random order.
  const rankToPlayer = new Map<number, Player>()

  // Assign relative ranks 1..K to seeded players
  seeded.forEach((p, i) => {
    rankToPlayer.set(i + 1, p)
  })

  // Fill remaining ranks with unseeded players in random order
  let unseededIdx = 0
  for (let rank = seeded.length + 1; rank <= N; rank++) {
